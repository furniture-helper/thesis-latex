...
The \texttt{OUT-OF-STOCK-TOKEN} was selected as most sites do not explicitly mention the availability of a product, but rather indicate that a product is out of stock when it is unavailable for purchase.
...
This was primarily due to the high cardinality of the token classes, with two classes for each product information category (e.g., \texttt{B-TITLE} and \texttt{I-TITLE} for the product title), which made it difficult for the model to learn the correct token classifications.
...
For this phase, only the extraction of the product title and price was considered, as the extraction of the product image and availability was considered the most important\footnote{While the extraction of the product image and availability is part of the functional requirements of the system, only the title and price extraction was considered for the model comparison and selection phase as expanding this models to include image and availability extraction would require additional resources and time, and the performance for title and price extraction should correlate with the performance for image and availability extraction as they are all token classification tasks. The complete performance of the final model used is described in Section~\ref{sec:information_extraction_evaluation}} for the proof-of-concept system.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{p{4cm} p{2.1cm} p{2.1cm} X X}
        ...
        \midrule
        Multi-modal &
        0.866\textsuperscript{1} &
        0.785\textsuperscript{1} &
        ...
        \begin{minipage}[t]{\linewidth}\centering 883 mins\textsuperscript{2}\\\$85.85\textsuperscript{2}\end{minipage} \\
        \midrule
        LLM (Gemini 2.5 Flash) &
        0.975 &
        0.960 &
        \begin{minipage}[t]{\linewidth}\centering 10 mins\\\$0.04\end{minipage} &
        \begin{minipage}[t]{\linewidth}\centering 1,576 mins\textsuperscript{2}\\\$281.68\textsuperscript{2}\end{minipage} \\
        \bottomrule
    \end{tabularx}
    \par\vspace{0.4em}
    \raggedright\footnotesize
    \textsuperscript{1} The performance of the multi-modal model could be improved with further training and fine-tuning, but not enough to justify the additional cost.\\
    \textsuperscript{2} Extraction time and cost for the LLM approaches are estimated based on a sample of 200 pages, and could be improved through parallelization.
...
$$
BEGIN
    INSERT INTO price_history (url, price, recorded_at)
...
DROP TRIGGER IF EXISTS trg_record_price_changes ON product_information;
