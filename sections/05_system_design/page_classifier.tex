...
The page classifier module is responsible for classifying the crawled pages into product pages and non-product pages.
...
\begin{itemize}
    ...
    \item \textbf{BERT embeddings with a neural network classifier:} This approach involves using a pre-trained BERT model to generate embeddings for the HTML content, which are then fed into a neural network classifier for classification.
    \item \textbf{MarkupLM:} This approach involves using a pre-trained MarkupLM model, which is specifically designed for processing HTML content, to classify the pages.
\end{itemize}
