...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{p{0.48\textwidth} p{0.48\textwidth}}
        ...
        \textbf{Not achieved.} p95 response time for similar product retrieval was \texttt{2,907ms}. This is because vector comparisons are computationally intensive, and a relatively small RDS instance (\texttt{db.t4g.small} with \texttt{2 vCPUs} and \texttt{2 GiB of memory}) was used due to cost constraints. \\
        \midrule
        The system must be implemented at a low cost of under USD 300 per month, including cloud hosting and storage costs. &
        \textbf{Achieved.} The system was implemented at a cost of \texttt{\$6.04} per day, which is equivalent to \texttt{\$181.20} per month.
         \\
        \bottomrule
    \end{tabularx}
